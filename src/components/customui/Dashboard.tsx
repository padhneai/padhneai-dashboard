...
// Lazy load AddSubjectForm for better initial performance
const AddSubjectForm = React.lazy(() => import("@/components/addsubject/Addsuject"));
...
interface DashboardProps {
  ...
  initialdata: {
    subjectdata: any;
    subjectAnalytics: SubjectAnalytics[] | null;
    dashboardanalytics: any;
  };
}

export default function Dashboard({ classid, classname, initialdata }: DashboardProps) {
  const [showAddSubject, setShowAddSubject] = useState(false);

  // Fetch subjects
  const { data: classdata, mutate: mutateSubjects, isLoading } = useSWR<(any)>(
    "/subjects",
    getAllSubjects,
    { fallbackData: initialdata.subjectdata }
  );

  // Fetch analytics
  const { data: subjectanalytics, mutate: subjectmutate, isLoading: isAnalyticsLoading } = useSWR<SubjectAnalytics[] | null>(
    "/subjects-analytics",
    getEachSubjectAnalytics,
    { fallbackData: initialdata.subjectAnalytics }
  );

  const { data: dashboardanalytics, isLoading: isdashboardLoading } = useSWR(
    "/dashboard-analytics",
    getDashboadAnalytics,
    { fallbackData: initialdata.dashboardanalytics }
  );

  // Memoize analytics map for fast lookup
  const analyticsMap = useMemo(() => {
    return subjectanalytics?.reduce((acc, item) => {
      acc[item.subject_id] = item;
      return acc;
    }, {} as Record<number | string, SubjectAnalytics>) || {};
  }, [subjectanalytics]);
  ...
  const isFetching = isLoading || isAnalyticsLoading || isdashboardLoading;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-6 mb-10">
        ...
        <AnalyticsCard title="10 Set Questions" value={total10set} icon={ClipboardCheck} colorFrom="from-teal-500" colorTo="to-teal-600" />
      </div>

      {/* Subjects Grid */}
      {isFetching ? (
        <div className="py-10">
          <DataDisplayLoading count={8} />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {classdata.results.map((subject: any) => (
            <SubjectCard
              ...
              classname={classname}
              classid={classid}
              subjectanalytics={analyticsMap[subject.id]}
            />
          ))}
        </div>
      ) : (
        <p className="text-center text-gray-500 py-20">
          No subjects found for this class. Add some subjects to get started.
        </p>
      )}
